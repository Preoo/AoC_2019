...
# Below is bunch of messy one-off code, for adults only.
...
# Utils with hardcoded paths 
def save_maze(maze):
    with open('./tmp/day_15.pickle', mode='wb') as f:
        pickle.dump(maze, f)

def load_maze():
    with open('./tmp/day_15.pickle', mode='rb') as f:
        maze = pickle.load(f)
    return maze

def print_maze(maze, dimensions):
    # dimensions is list of tuples, for upper-left corner and bottom-right corner
    for row in range(dimensions[0][0], dimensions[1][0] + 1):
        for column in range(dimensions[0][1], dimensions[1][1] + 1):
            print(maze[(row, column)], end='')
        print('')

# Use robot to get graph G of maze and O2 system || do it by hand
...
# Used to create pickle-able default factory for defaultdict
def maze_construct():
    return ' '
maze = defaultdict(maze_construct)
maze_dims = [(-3,-3), (3, 3)] #top-left, bottom-right, where (0,0) is center
...
drone = IntCodeMachine(program, memory=10000)
drone_call = iter(drone)
drone_pos = (0,0)
maze[drone_pos] = drone_glyph
O2_pos = (12, -12)
if O2_pos is None:
    while True:
        # Find O2 and maze by hand, decided to do this instead of random, bfs etc since I assumed that
        # maze could contain loops and other traps. However it looks like to be 1 space wide corridors,
        # BFS or following the wall could have been enough. Thankfully maze was small enough.
        # Worst case: on junction, save unexplored and run A* to return to it once current path ends in wall.
        print('Order: w,s,a,d - up,down,left,right - exit(e, without saving(q))')
        command = str(msvcrt.getch(), encoding='utf-8') # Needs encoding, default is bytes, without raises keyerror against commands-dict
        ...
        if command == 'e':
            ...
            c = commands[command]
            pos_modifier, bot_input = c['pos'], c['bot']

            drone_new_pos = tuple(old + new for old, new in zip(drone_pos, pos_modifier))
            if abs(drone_new_pos[0]) >= abs(maze_dims[0][0]) or abs(drone_new_pos[1]) >= abs(maze_dims[1][1]):
                maze_dims = [(maze_dims[0][0] - 1, maze_dims[0][1] - 1), (maze_dims[1][0] + 1, maze_dims[1][1] + 1)]
            
            drone.feed_inputs(bot_input)
            while len(drone.io_o) < 1:
                next(drone_call)
            ...
            if status == 0:
                print('Hit wall')
                maze[drone_new_pos] = wall_glyph

            elif status > 0:
                print('Moved to new position')
                maze[drone_pos] = floor_glyph
                drone_pos = drone_new_pos
                maze[drone_new_pos] = drone_glyph

                if status == 2:
                    O2_pos = drone_new_pos
                    maze[O2_pos] = O2_glyph
                    print(f'Found O2 system at {O2_pos}') # O2 is at (12, -12)
            print_maze(maze, maze_dims)

# This is BFS, random, by-hand populated map of puzzle maze. Hardcoded path/file.
try:
    maze = load_maze()
except OSError:
    print(f'Error while loading maze. File is missing?')
    raise

def adjacent_nodes(graph, w, condition=lambda u: True):
    # used to create adjancy graph for this task
    # if there is reuse then this needs to be cleaned
    # condition: a function which returns True if value is accepted.
    def dist(a, b):
        ax, ay = a
        bx, by = b
        # This is so dumb ...
        if ax - 1 == bx and ay == by:
            return True
            ...
            return False
    for u in graph:
        if dist(u, w) and condition(u):
            yield u

# Generate adjacent_list from maze nodes. Match only floors, marked with '.'
# Resulting graph is used with Dijkstras below.
graph = {
    node : list(adjacent_nodes(maze, node, condition=lambda u: maze[u] == '.'))
    for node in maze
    if maze[node] == '.'
    }

# Dijkstra https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm Targeted for this problem
def Dijkstra(graph:dict, source:tuple) -> dict:
    open_set = set(graph.keys())
    distance = {source : 0}
    while len(open_set):
        # node u with min distance to source not in open_set
        u = min(open_set, key=lambda i: distance.get(i, inf))
        open_set.remove(u)
        # neightbours of u which aren't in open_set
        for v in [n for n in graph[u] if n in open_set]:
            alt = distance.get(u, inf) + 1 # distance to neightbour is always 1
            if alt < distance.get(v, inf):
                distance[v] = alt

    return distance

dijkstra = Dijkstra(graph, (12,-12))
# Correct answer is 244
print(f'Part 1 answer: {dijkstra[(0,0)]} commands to reach O2')

# Last spot to be gassed is biggest leaf in dijkstra
# Correct answer is 278
print(f'Part 2 answer: {max(dijkstra.values())} minutes to saturate')